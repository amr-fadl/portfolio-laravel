...
@section('pageTitle' , __('All').' '.__('Education'))
...
@section('content')
    ...
    <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between">
            ...
            <div class="order-3">
                <a href="{{ route('education.create') }}" class="btn btn-info w-100">{{ __('Add') }} {{  __('Education')  }}</a>
            </div>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
                            ...
                            <thead>
                                <tr role="row">
                                    ...
                                    <th class="sorting_asc" tabindex="0" aria-controls="example1" rowspan="1"
                                        ...
                                        aria-label="Rendering engine: activate to sort column descending">{{__('Title')}}</th>

                                    <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1"
                                        colspan="1" aria-label="Browser: activate to sort column ascending">{{__('Date')}}</th>
                            ...
                            <tbody>
                                @foreach ($allData as $singl)
                                    <tr role="row" class="amr">
                                        ...
                                        <td class="align-middle">{{$singl->date}}</td>
                                        ...
                                        <td class="align-middle" style="max-width:30%; width:250px">
                                            <div class="d-flex flex-wrap" style="row-gap: 10px">
                                                <div class="col-12 col-sm-6">
                                                    <a href="{{ route('education.edit', ['education' => $singl->id]) }}" class="btn btn-info w-100">{{ __('Edit') }}</a>
                                                </div>
                                                <div class="col-12 col-sm-6">
                                                    <form method="POST" class="w-100"
                                                        action="{{ route('education.destroy', $singl->id) }}">
                                                        @csrf
                                                        @method('DELETE')
