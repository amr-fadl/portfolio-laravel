    ...
    <div class="row">
        <div class="col-md-6">
            <div class="card card-secondary">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('skill.update' , $skill->id) }}" class="myform" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="mt-3 d-flex justify-content-center">
                            <img height="250px" class="_img" src="{{ asset('uploads/frontend/skills/'.$skill->image) }}" alt="">
                        </div>
                        <div class="mt-3">
                            <label for="inputName">Name</label>
                            <input type="text" name="name" value="{{ $skill->name }}" id="inputName" class="form-control">
                            <x-input-error :messages="$errors->get('name')" class="errors" />
                        </div>
                        <div class="mt-3">
                            <label for="inputexperience">percentage</label>
                            <input type="number" min="1" max="100" name="percentage" value="{{ $skill->percentage }}" id="inputpercentage" class="form-control">
                        ...
                        <div class="mt-3">
                            <select name="order_skill" class="form-control wide">
                                @foreach ($orderSkill as $key => $value )
                                    <option value="{{ $key+1 }}" {{$skill->order_skill == $key+1 ? 'selected' : '' }}>{{ $value }}</option>
                                @endforeach
                            </select>
...
@endsection
@section('pageTitle', __('Edit Skill'))
