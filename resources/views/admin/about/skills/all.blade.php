...
@section('pageTitle' , __('All').' '.__('Skill'))
...
@section('content')
    ...
    <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between">
            ...
            <div class="order-3">
                <a href="{{ route('skill.create') }}" class="btn btn-info w-100">{{ __('Add') }} {{  __('Skill')  }}</a>
            </div>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
                        ...
                        <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" role="grid"
                            ...
                            <thead>
                                <tr role="row">
                                    ...
                                    <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1"
                                        colspan="1" aria-label="Browser: activate to sort column ascending">{{__('Percentage')}}</th>

                                    <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1"
                                        colspan="1" aria-label="Browser: activate to sort column ascending">{{__('Order Skill')}}</th>
                            ...
                            <tbody>
                                @foreach ($allData as $singl)
                                    <tr role="row" class="amr">
                                        ...
                                        <td class="align-middle"><img src="{{ asset('uploads/frontend/skills/'.$singl->image) }}" style="height: 100px; width:100px; object-fit: cover;" alt=""></td>
                                        <td class="align-middle">{{$singl->name}}</td>
                                        <td class="align-middle">{{$singl->percentage}}</td>
                                        <td class="align-middle">{{$singl->order_skill}}</td>
                                        <td class="align-middle" style="max-width:30%; width:250px">
                                            <div class="d-flex flex-wrap" style="row-gap: 10px">
                                                <div class="col-12 col-sm-6">
                                                    <a href="{{ route('skill.edit', ['skill' => $singl->id]) }}" class="btn btn-info w-100">{{ __('Edit') }}</a>
                                                </div>
                                                <div class="col-12 col-sm-6">
                                                    <form method="POST" class="w-100"
                                                        action="{{ route('skill.destroy', $singl->id) }}">
                                                        @csrf
                                                        @method('DELETE')
