...
@section('pageTitle' , __('All').' '.__('Filter'))
...
@section('content')
    ...
    <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between">
            ...
            <div class="order-3">
                <a href="{{ route('filter.create') }}" class="btn btn-info w-100">{{ __('Add') }} {{  __('Title')  }}</a>
            </div>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
                                        ...
                                        <td class="align-middle" style="max-width:30%; width:250px">
                                            <div class="d-flex flex-wrap" style="row-gap: 10px">
                                                <div class="col-12 col-sm-6 mt-2">
                                                    <a href="{{ route('filter.edit', $singl->id) }}" class="btn btn-info w-100">{{ __('Edit') }}</a>
                                                </div>
                                                <div class="col-12 col-sm-6 mt-2">
                                                    <form method="POST" class="w-100"
                                                        action="{{ route('filter.destroy', $singl->id) }}">
                                                        @csrf
                                                        @method('DELETE')
