    ...
    <div class="row">
        <div class="col-md-6">
            <div class="card card-secondary">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('portfolio.update', $portfolio->id) }}" class="myform" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="mt-3 d-flex justify-content-center">
                            <img height="250px" class="_img" src="{{ asset('uploads/frontend/Portfolio/'.$portfolio->image) }}" alt="">
                        </div>
                        <div class="mt-3">
                            <label for="inputName">Name</label>
                            <input type="text" name="name" value="{{$portfolio->name}}" id="inputName" class="form-control">
                            <x-input-error :messages="$errors->get('name')" class="errors" />
                        </div>
                        <div class="mt-3">
                            <label for="inputexperience">mini_description</label>
                            <input type="text" min="1" max="100" name="mini_description" value="{{$portfolio->mini_description}}" id="inputmini_description" class="form-control">
                            <x-input-error :messages="$errors->get('mini_description')" class="errors" />
                        </div>
                        <div class="mt-3">
                            <label for="inputDescription">Description</label>
                            <textarea id="description" name="description" class="textarea_editor form-control" rows="4">{{ $portfolio->description }}</textarea>
                            <x-input-error :messages="$errors->get('description')" class="errors" />
                        </div>
                        <div class="mt-3">
                            <select name="filter_portfolio" class="form-control wide">
                                @foreach ($filter as $value )
                                    <option value="{{ $value->id }}" {{($portfolio->filters->first()->id??1) == $value->id ? 'selected' : ''}} >{{ $value->name }}</option>
                                @endforeach
                            </select>
