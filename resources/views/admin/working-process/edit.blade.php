    ...
    <div class="row">
        <div class="col-md-6">
            <div class="card card-secondary">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('workingProcess.update' , $workingProcess->id) }}" class="myform" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="mt-3 d-flex justify-content-center">
                                <img src="{{asset('uploads/frontend/working_processes/'.$workingProcess->icon)}}" alt="" class="_img">
                        </div>
                        <div class="mt-3">
                            <label for="inputName">{{__('Title')}}</label>
                            <input type="text" name="title" value="{{$workingProcess->title}}" id="inputTitle" class="form-control">
                            <x-input-error :messages="$errors->get('title')" class="errors" />
                        </div>
                        <div class="mt-3">
                            <label for="inputName">{{__('Name')}}</label>
                            <input type="text" name="name" value="{{$workingProcess->name}}" id="inputname" class="form-control">
                            <x-input-error :messages="$errors->get('name')" class="errors" />
                        </div>
                        <div class="mt-3">
                            <label for="inputDescription">{{__('Description')}}</label>
                            <textarea id="description" name="description" class="textarea_editor form-control" rows="4">{{$workingProcess->description}}</textarea>
