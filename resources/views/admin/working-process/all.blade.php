...
@section('pageTitle' , __('All').' '.__('working Process'))
...
@section('content')
    ...
    <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between">
            ...
            <div class="order-3">
                <a href="{{ route('workingProcess.create') }}" class="btn btn-info w-100">{{ __('Add') }} {{  __('working Process')  }}</a>
            </div>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
                            ...
                            <thead>
                                <tr role="row">
                                    ...
                                    <th class="sorting_asc" tabindex="0" aria-controls="example1" rowspan="1"
                                        ...
                                        aria-label="Rendering engine: activate to sort column descending">{{__('icon')}}</th>
                            ...
                            <tbody>
                                @foreach ($allData as $singl)
                                    <tr role="row" class="amr">
                                        ...
                                        <td class="align-middle"><img src="{{ asset('uploads/frontend/working_processes/'.$singl->icon) }}" alt=""></td>
                                        ...
                                        <td class="align-middle" style="max-width:30%; width:250px">
                                            <div class="d-flex flex-wrap" style="row-gap: 10px">
                                                <div class="col-12 col-sm-6 mt-2">
                                                    <a href="{{ route('workingProcess.edit', ['workingProcess' => $singl->id]) }}" class="btn btn-info w-100">{{ __('Edit') }}</a>
                                                </div>
                                                <div class="col-12 col-sm-6 mt-2">
                                                    <form method="POST" class="w-100"
                                                        action="{{ route('workingProcess.destroy', $singl->id) }}">
                                                        @csrf
                                                        @method('DELETE')
