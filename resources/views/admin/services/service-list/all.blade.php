...
@section('content')
    ...
    <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between">
            ...
            <div class="order-3">
        <a href="{{ route('servicelist.create' , ["id" => $allData->id]) }}" class="btn btn-info w-100">{{ __('Add') }} {{  __('List')  }}</a>
            </div>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
                        ...
                        <table id="example1" class="table table-bordered table-striped dataTable dtr-inline" role="grid"
                            ...
                            <thead>
                                <tr role="row">
                                    ...
                                    <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1"
                                        colspan="1" aria-label="Browser: activate to sort column ascending">{{__('Service')}}</th>
                            ...
                            <tbody>
                                @foreach ($allData->ServicesList as $singl)
                                    <tr role="row" class="amr">
                                        ...
                                        <td class="align-middle">{{$allData->title}}</td>
                                        <td class="align-middle" style="max-width:30%; width:250px">
                                            <div class="d-flex flex-wrap" style="row-gap: 10px">
                                                <div class="col-12 col-sm-6 mt-2">
                                                    <a href="{{ route('servicelist.edit', $singl->id) }}" class="btn btn-info w-100">{{ __('Edit') }}</a>
                                                </div>
                                                <div class="col-12 col-sm-6 mt-2">
                                                    <form method="POST" class="w-100"
                                                        action="{{ route('servicelist.destroy', ['servicelist'=>$singl->id]) }}">
                                                        @csrf
                                                        @method('DELETE')
